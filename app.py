...
logger = logging.getLogger('Logging System by snare')
...
def get_public_ip():
    ...
    if public_ip:
        logger.info('Got a public IP: %s', public_ip)
        return public_ip
    else:
...
class IPValidator(Validator):
    def validate(self, document):
        ...
        try:
            ip = document.text
            if ipv4.match(ip):
                logger.info('IP matched in validator: %s', ip)
    ...
    def validate_get(text):
        ...
        try:
            ip = text
            if ipv4.match(ip):
                logger.info('IP validated in validate_get: %s', ip)
                ipaddress.IPv4Address(ip)
